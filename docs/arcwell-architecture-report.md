...
- The preferred distribution shape is open-sourceable service packages such as `arcwell-memory`, `arcwell-llm-wiki`, `arcwell-telegram`, `arcwell-x`, `arcwell-mail`, and a shared `arcwell-channel-kit`, not a private monolithic "personal Codex" fork/plugin.
...
Use one consistent placement rule: capabilities are open-source `arcwell-*` packages; stateful work runs in local daemons or bounded Cloudflare Workers; agents interact through MCP and skills; host-specific glue lives under host adapters.

| Surface | Owns | Does not own | Examples |
|---|---|---|---|
| `arcwell-*` package | Distribution boundary, docs, schemas, fixtures, threat model, host adapters | Runtime authority by itself | `arcwell-memory`, `arcwell-llm-wiki`, `arcwell-garderobe` |
...
| CLI | Setup, import/export, doctor, backup/restore, migrations, one-shot admin/debug commands | Background listening, agent reasoning, long-lived state mutation without logs | `arcwell memory doctor`, `arcwell wiki ingest`, `arcwell backup restore-plan` |
...
- Avoid the word "addon" for the new architecture except when referring to codex-swift history. The target unit is an `arcwell-*` service package.
...
6. Is it a Codex thread/control operation?
   Put it in `arcwell-controller` plus `hosts/codex`, with a degraded read-only MCP path for other agents.
...
- Email Workers for inbound email routing to code; this makes `arcwell-mail` possible without running an always-on mail server.
...
- `worker` code can enqueue, validate, and serve small remote MCP tools, but should not fork a second copy of local business logic unless the package is intentionally edge-native, as with `arcwell-garderobe`.
...
- `arcwell-envelope`: canonical event envelope, idempotency key, source identity, payload limits, provenance fields.
- `arcwell-channel`: shared channel contract for inbound/outbound messages, prompt-injection fencing, formatting, attachments, receipts, and routing policy.
- `arcwell-search`: optional search-provider abstraction for host-native web search, OpenAI web search/deep-research, Claude web search, Brave Search, Perplexity Search/Sonar, and future providers.
- `arcwell-edge-protocol`: Cloudflare auth helpers, D1 schemas, queue/R2 helpers, retention enforcement, and generated TypeScript bindings for Workers.
- `arcwell-local`: drain protocol, ack/retry, local secret loading, Codex SDK/app-server client wrapper.
- `arcwell-mcp`: common MCP error shapes, job resources, async job status conventions.
- `arcwell-backup`: encrypted snapshot, restore, integrity-check, and retention helpers for local SQLite stores, wiki files, ledgers, and configs.
- `arcwell-secrets`: credential storage/refresh/expiry/rotation contract across OS keychain, 0600 local files, and scoped Cloudflare secrets.
- `arcwell-cost`: per-job/model/provider token and spend ledger, budgets, estimates, alerts, and kill switches.

Recommended open-source apps:

| Package | Edge component | Local component | Agent surface |
|---|---|---|---|
| `arcwell-memory` | Usually none; optional encrypted edge backup/sync later | Personal mem0-style memory via `mem0-rs`, SQLite, and MCP/HTTP | Personal memory MCP + review skills |
| `arcwell-profile` | Usually none; optional encrypted backup/sync later | Inspectable preference/profile store for output style, recurring constraints, taste, sizes/settings, decision criteria, and support preferences | Profile/preference MCP + host prompt hints |
| `arcwell-backup` | Optional encrypted R2/object-store snapshots | Local encrypted versioned backups, restore, integrity checks, stale-backup alerts | Backup/restore MCP + doctor checks |
| `arcwell-secrets` | Scoped edge secret metadata/health only | OS keychain/0600 secret abstraction, OAuth refresh/expiry/rotation, grant health | Credential health MCP + setup/doctor skills |
| `arcwell-cost` | Optional edge job cost mirror | Local cost ledger, per-package/source/model budgets, run estimates, global kill switch | Cost/status MCP + ops UI cards |
| `arcwell-llm-wiki` | Capture endpoints, optional Browser Run fetch/render, queue/R2 staging, remote MCP for small queries | Knowledge corpus store, ingest pipeline, embeddings/extraction, UI/API, queue drain | Wiki MCP + research/ingest skills |
| `arcwell-telegram` | Telegram webhook, owner validation, queue, optional outbound send relay | Codex thread bridge, workspace policy, local approvals, streaming/replies | Skills for channel policy; MCP for send/status |
| `arcwell-controller` | Optional status inbox and remote-control front door; no public write surface by default | Project/thread registry, active-run monitor, summary index, natural-language router, Codex SDK/app-server client | MCP tools for project/thread status, routing, creation, resume/follow-up |
| `arcwell-ops-ui` | Optional read-only Cloudflare status mirror for always-on health | Local web UI for ingestion, jobs, history, logs, health, recent errors, and service config | MCP status/card tools first; in-app browser preview for visual/interactive inspection |
| `arcwell-conversation-import` | Usually none; optional encrypted import staging later | Consentful Claude/Codex export summarizer, redactor, profile/wiki/source-card writer, import ledger | Import/review MCP + migration skills |
| `arcwell-decision-support` | Optional price/watch capture later | Recommendation rationale ledger, option comparisons, purchase/return state, sizing/fit decisions, source snapshots | Compare/decide/revisit skills |
| `arcwell-routine-context` | Weather/calendar/location capture where explicitly configured | Opt-in daily context for weather, calendar, routines, health-adjacent constraints, outfit/travel planning | Daily brief/planning MCP |
| `arcwell-x` | OAuth callback, cron monitor, cursor/spend state, queue of imported items | Wiki writer, local enrichment, reports | X MCP + monitor/report skills |
| `arcwell-mail` | Email Worker, attachment staging, loop prevention metadata, queue | Mail drain, Gmail/IMAP/workspace archive writer, optional Codex thread bridge | Mail capture MCP + triage skills |
| `arcwell-garderobe` | Existing Cloudflare Worker/D1/OAuth remote MCP copied from `/Users/chabotc/Projects/garderobe` | Optional local mirror/export later | Wardrobe/outfit MCP + weather/style skills |
| `arcwell-content` | Optional publish/webhook capture for comments/social analytics | Writing style profile, editorial calendar, blog/social draft pipeline, asset ledger | Blog/post/report/slides/video skills + wiki/style MCP |
| `arcwell-workspace-context` | Optional Cloudflare email/docs capture inbox | Google Docs/Gmail/Contacts/Drive context index via official connectors first | Workspace-to-wiki capture + relationship/context skills |
| `arcwell-browser-workflows` | Usually none; optional remote browser capture | Browser/computer-use profiles, task recipes, screenshot/action ledgers | Browser workflow skills using host browser/computer tools |
| `arcwell-media` | Optional asset delivery/webhook callbacks | Slide/video/image generation ledger, source assets, exports | Media generation/publishing skills |
| `arcwell-deep-research` | Optional Cloudflare capture/queue for research seeds, scheduled checks, and source snapshots | Mostly none; uses host-agent native web search, Codex/Claude research workflows, wiki MCP, and optional search adapters | Deep-research skills + optional custom agents |
| `arcwell-channel-kit` | Shared Cloudflare helpers for webhook validation, queueing, delivery receipt storage | Shared local interfaces for channel drains, formatting, prompt-injection protection, and delivery outcomes | No generic app by default; concrete channels expose tools |
| `arcwell-search-kit` | Usually none; optional remote provider proxy if keys should stay off laptops | Optional MCP/search adapter wrapping Brave Search, Perplexity Search/Sonar, and provider result normalization | Search MCP tools/resources usable by Codex and Claude Desktop |
| `arcwell-quality-kit` | None | Reusable verification contracts, acceptance checks, replay fixtures, and source/visual/test validation helpers | Skills for bug reproduction, tests, citations, visual inspection, uncertainty surfacing |
| `arcwell-voice-mobile` | Optional channel front door for voice/mobile capture | Mostly none at first; uses concrete channel packages and controller | Voice/mobile capture hints and notification policies |
| `arcwell-runtime` | Shared only after repeated patterns appear | Shared only after repeated patterns appear | Installer/doctor helpers |

This lets other people adopt only what they need. Someone can run just `arcwell-llm-wiki` locally, or deploy `arcwell-telegram` on Cloudflare with their own bot token, without inheriting your X/Gmail/memory choices.
...
|---|---|---|---|---|---|---|
| `arcwell-profile` | Optional lightweight profile service; can start file/SQLite-backed | edit/import/export/doctor | read/search/update/review profile slices | when to consult profile; profile hygiene | backup only | prompt hints and relevant-slice loading |
| `arcwell-memory` | Required for capture/reconcile/dream jobs | import/export/review/debug/bench | recall, add/update/delete, lifecycle jobs | memory review/capture/dream/debug | optional encrypted backup later | Codex hooks where available; Claude manual capture |
| `arcwell-llm-wiki` | Required for ingest/search/index/librarian | ingest/reindex/reembed/audit | search/read/ingest/job/status/source cards | research/ingest/librarian workflows | capture inbox, Browser Run snapshots, remote small queries | host search/wiki guidance |
| `arcwell-controller` | Required for project/thread registry and Codex bridge | project/thread admin and doctor | status, resolve, continue, create, approvals | channel routing and status workflows | optional read-only status/front door | Codex app-server/SDK bridge; Claude read-only/status |
| `arcwell-channel-kit` | Library, not normally a daemon | fixture/replay helpers | only shared schemas if useful | channel safety guidance | shared webhook/receipt helpers | shared host docs |
| `arcwell-telegram` | Local bridge when driving Codex/local projects | setup/test-send/replay | send/status/thread map | Telegram policy and formatting | webhook, validation, queue, outbound relay | controller integration |
| `arcwell-mail` | Local drain/processor if mail becomes a channel | setup/replay/archive | mail capture/status/send-draft where allowed | triage/archive workflows | Email Worker, attachment staging | Gmail/Workspace connector hints |
| `arcwell-x` | Local enricher/wiki writer or Worker-native monitor by policy | oauth/import/report/replay | import/status/report | signal/report workflows | OAuth callback, cron/cursors/queue | wiki/content integration |
| `arcwell-content` | Usually not a daemon unless publishing queues exist | draft/export/publish-dry-run | style read, draft, rewrite, variants, calendar | writing/social/report workflows | optional analytics/comment webhooks | Codex/Claude writing guidance |
| `arcwell-garderobe` | Optional local mirror later | import/export/audit | outfit/wardrobe/weather/packing tools | outfit/packing/audit workflows | primary remote MCP/D1/OAuth app | Codex/Claude MCP setup |
| `arcwell-ops-ui` | Required local ops/status service | doctor/logs/open-ui | health/jobs/errors/logs/approvals/cards | ops status/diagnose/ui workflows | optional read-only status mirror | browser-use guidance |
| `arcwell-backup` | Scheduled local snapshot service | backup/verify/restore-plan/restore | status/create/verify/restore-plan | backup hygiene | optional encrypted R2 target | doctor warnings |
| `arcwell-secrets` | Usually library plus health checker | login/logout/rotate/doctor | credential health, not secret readout | setup/recovery guidance | scoped edge secret metadata | connector setup docs |
| `arcwell-cost` | Local ledger/guard | report/budget/kill-switch | cost summaries/budget controls | cost-aware effort guidance | optional status mirror | effort-router integration |
| `arcwell-deep-research` | No daemon initially | run/export/eval optional | uses wiki/search MCP rather than many custom tools | primary surface | optional seed capture | host-native search/subagent instructions |
...
- Never require all `arcwell-*` servers in one Codex/Claude session. Load by workflow: coding, personal profile/memory, wiki/research, channel ops, wardrobe, or media.
...
Do not build `arcwell-push`. "Push" is not a domain boundary. The real abstraction is a channel with inbound and outbound behavior, platform identity, formatting rules, prompt-injection controls, and delivery receipts.

Use `arcwell-channel-kit` as the shared package and keep concrete platforms in their own packages:

- `arcwell-telegram`: Telegram Bot API, chats, users, groups, message formatting, files, bot-token/webhook quirks.
- `arcwell-discord`: Discord users/guilds/channels/threads, slash commands, embeds, markdown, attachments, permissions.
- `arcwell-slack`: Slack workspaces/channels/threads, Block Kit, app mentions, interactive actions, workspace auth.
- `arcwell-mail` or `arcwell-gmail-channel`: inbound/outbound email, MIME, threading, loop prevention, reply quoting, unsubscribe/auto-reply hazards.
...
`arcwell-channel-kit` should define:
...
- Build `arcwell-ops-ui` as a standalone local web app served by the local daemon/controller.
...
`json-render` can fit, but not as a magic Codex Desktop extension. It is useful if we want a constrained, schema-driven renderer inside `arcwell-ops-ui` or generated HTML artifacts:
...
1. `arcwell-conversation-import` reads the export and creates low-authority summaries.
2. It proposes candidates for `arcwell-profile`, `arcwell-memory`, `arcwell-llm-wiki`, and `arcwell-decision-support`.
...
- Restore must be tested, not assumed. `arcwell-backup` should include `backup_create`, `backup_status`, `backup_verify`, `backup_restore_plan`, and `backup_restore`.
...
- `arcwell-content` should show source lineage before publishing: wiki pages, source cards, retrieved URLs, model/prompt versions, and any risky instructions found in source text.
...
- For personalized tasks, host adapters should consult `arcwell-profile`/`arcwell-memory`/`arcwell-llm-wiki` as appropriate before answering, then provide a small consultation receipt when it helps trust.
...
- Add a global kill switch and per-package pause switches surfaced in `arcwell-ops-ui`.
...
That is the easiest portable default for `arcwell-deep-research`: fewer extra API keys, fewer moving parts, and the best citation/search behavior each agent already knows how to use.

Add optional expansion providers through `arcwell-search-kit`:
...
`arcwell-search-kit` should normalize all providers into source cards:
...
- The proposed `arcwell-*` family should leave Codex as the human-facing agent/runtime and add reachability, memory, event capture, and external delivery around it.
...
- Core/app boundary: shared runtime owns queueing, idempotency, auth validation, retries, receipts, and generic MCP shapes; `arcwell-channel-kit` owns common channel safety/formatting/receipt contracts; each concrete app owns platform quirks such as Telegram threads, Discord embeds, Slack Block Kit, Gmail loop prevention, X rate limits, or email MIME parsing.
- Controller boundary: `arcwell-controller` owns project/thread discovery, status, summaries, natural-language routing, and creation/resume operations. Channel packages should call the controller instead of duplicating workspace routing logic.
...
- Setup/migration importers: `arcwell-llm-wiki` can import current codex-swift wiki state; `arcwell-telegram` can import bot/thread mappings; `arcwell-x` can import OAuth/cursor state after explicit confirmation.
...
- Personal services such as wardrobe planning should live in the same arcwell service family, not as isolated one-offs.
...
Product implication: add an inspectable personal profile/preference layer next to `arcwell-memory`. Mem0-style memory is good for compact facts and evolving personal memory, but durable operating preferences such as writing style, communication register, sizing defaults, wardrobe rules, approval friction, and recommendation criteria should be visible documents/resources the user can audit and edit.
...
1. `arcwell-profile`: portable personal profile and preference store.
   ...
   - Boundary: `arcwell-memory` remembers facts and learns from conversation; `arcwell-profile` is the explicit, curated operating manual.

2. `arcwell-conversation-import`: consentful history import.
...
3. `arcwell-decision-support`: remembered reasoning, not one-off advice.
   ...
   - Shared by `arcwell-garderobe`, `arcwell-content`, `arcwell-workspace-context`, and research workflows.

4. `arcwell-content`: content production and publishing support.
...
5. `arcwell-workspace-context`: Google Workspace as context, not reimplemented plumbing.
...
6. `arcwell-garderobe`: copy the existing wardrobe MCP into the monorepo/package family.
...
7. `arcwell-routine-context`: opt-in daily context.
...
8. `arcwell-browser-workflows`: browser/computer-use fallback.
...
9. `arcwell-media`: decks, images, videos, and asset ledgers.
...
10. `arcwell-quality-kit`: reusable verification discipline.
...
11. `arcwell-signal` as an internal module of `arcwell-llm-wiki`.
...
|---|---|---:|---|
| Personal memory | `arcwell-memory` | P0 | Gives all agents durable personal context and low-friction capture. |
| Explicit personal profile | `arcwell-profile` | P0 | Keeps tone, preferences, recurring constraints, decision criteria, and support expectations inspectable instead of hidden. |
| Knowledge/wiki/signal | `arcwell-llm-wiki` | P0 | Central substrate for research, content, monitoring, and trend reports. |
| Project/thread controller | `arcwell-controller` | P0 | Lets Telegram/mobile ask about active Codex work and route follow-ups. |
| Safe history migration | `arcwell-conversation-import` | P0 | Converts Claude/Codex exports into reviewed profile, memory, wiki, and decision artifacts without raw transcript sludge. |
| Quality discipline | `arcwell-quality-kit` | P0 | Preserves root-cause debugging, tests, citations, visual inspection, and explicit uncertainty across packages. |
| Content/style/publishing | `arcwell-content` | P1 | Turns wiki/signal into blog posts, social posts, reports, slides, and drafts in the user's voice. |
| Decision support | `arcwell-decision-support` | P1 | Makes recommendations reusable by remembering constraints, alternatives, sources, and why a choice was made. |
| Channels | `arcwell-telegram`, `arcwell-mail`, `arcwell-channel-kit` | P1 | Makes the system reachable and proactive without generic push. |
| Workspace context | `arcwell-workspace-context` | P1 | Connects Google Docs, Gmail, Calendar, Contacts, and Drive to projects and wiki. |
| Wardrobe | `arcwell-garderobe` | P1 | Existing Cloudflare MCP is ready to copy into the family and is the flagship personal-domain app. |
| Routine/daily context | `arcwell-routine-context` | P1 | Lets calendar/weather/location/routine context support outfits, daily briefs, travel, and planning. |
| Browser workflows | `arcwell-browser-workflows` | P2 | Covers no-API workflows and visual tasks, but must be sandboxed and logged. |
| Media/decks/video | `arcwell-media` | P2 | Useful for project sites, reports, slides, social, and video, but depends on content/wiki foundation. |
| Mobile/voice front door | `arcwell-voice-mobile` | P2 | Useful for capture and notifications, probably built first on concrete channel packages rather than a new shell. |

## De-Portability Matrix

| codex-swift capability | Best regular-Codex home | Portability | Notes |
|---|---|---:|---|
| Personal mem0 memory | `arcwell-memory` using `mem0-rs` + SQLite + MCP + review skills | High | For durable personal facts/preferences such as pets, ADHD context, medications, communication preferences, and recurring task defaults. Keep separate from wiki corpora. |
| Conversation-derived profile/preferences | `arcwell-profile` + `arcwell-conversation-import` | High | Claude/Codex exports should become reviewed profile/memory/wiki/decision artifacts, not raw hidden context. |
| Decision/rationale history | `arcwell-decision-support` | High | Wardrobe, purchases, research choices, content angles, and technical options need reusable "why this, not that" records. |
...
| Project/thread meta-controller | `arcwell-controller` local service + MCP + Codex SDK/app-server | High | Needed for channel commands like "how is the de-porting project going?", "and the video project?", and "create a new project". |
...
| Channel inbound/outbound contract | `arcwell-channel-kit` + concrete channel apps such as Telegram/Discord/Slack/mail | High | Replace vague push with OpenClaw-style channel boundaries: inbound, outbound, formatting, prompt-injection protection, receipts, and platform identity. |
...
| Ops UI for ingest/status/logs/health | `arcwell-ops-ui` MCP status tools + standalone local web app | High | MCP is the primary agent interface. Use in-app browser/browser-use only for visual inspection, screenshots, and interactive UI workflows. |
...
regular Codex
  ...
  |
  +-- arcwell-channel-kit shared package
  |     +-- inbound/outbound message contract
  |     +-- prompt-injection and formatting rules
  |
  +-- arcwell-mail / arcwell-discord / arcwell-slack / other channels
  |
  +-- arcwell-deep-research plugin
...
- Each `arcwell-*` package: installable MCP server/service with app-specific docs plus host adapters for Codex skills/plugins and Claude MCP configuration.
- Optional shared `arcwell-runtime`: only after duplicated drain/envelope/auth code appears in at least two apps.
...
- UI: keep wiki content as Markdown files for Obsidian. Build `arcwell-ops-ui` for operational state, not as a Codex app-server fork. It can call `wikid`, `memd`, and controller HTTP APIs.
...
- Use Rust crates for the common runtime: `arcwell-envelope`, `arcwell-channel`, `arcwell-search`, `arcwell-edge-protocol`, `arcwell-local`, `arcwell-mcp`, and `arcwell-config`. Generate JSON Schema/OpenAPI from Rust types for Cloudflare Workers and browser UI clients.
...
- Package local daemons as single binaries with subcommands such as `arcwell-memory serve`, `arcwell-memory doctor`, `arcwell-llm-wiki serve`, `arcwell-controller serve`, and installable launchd/systemd templates.
...
|---|---:|---|
| `arcwell-memory` | Excellent | `mem0-rs` is already the right core: single binary, SQLite/history, REST, procedural memory, low RSS. |
| `arcwell-llm-wiki` / `wikid` | High | Rust is strong for file watching, bounded fetch/decode, SQLite/FTS/vector indexes, job ledgers, and large corpus processing. Use hosted models over HTTP. |
| `arcwell-controller` | High | Mostly SQLite state, Codex app-server/SDK protocol calls, summaries, routing, and MCP. If official Codex SDK support is JS-only, implement the narrow app-server HTTP/SSE/JSON-RPC client in Rust or shell out only as a temporary adapter. |
| `arcwell-telegram` / channel daemons | High | Telegram webhooks/long-poll, formatting, queues, owner policy, and receipts map cleanly to Rust HTTP clients and SQLite. |
| `arcwell-x` | High | OAuth state, cursors, polling, rate/spend ledgers, and wiki writes are good Rust sidecar work. Cloudflare may still own public OAuth callbacks. |
| `arcwell-mail` | Medium-high | MIME parsing, loop prevention, attachment staging, and channel delivery are fine in Rust; Gmail/Google APIs are more ergonomic through generated REST clients or a small HTTP layer than through a big dynamic discovery client. |
| `arcwell-ops-ui` backend | High | Axum can serve the local API, static assets, and MCP-backed status views. The browser frontend may still be TypeScript/React or a generated/static UI. |
...
Personal memory is mem0-shaped. It stores durable facts about the user, preferences, recurring context, and life details: "my cat is called Ophelia", "I have ADHD", "I use these medications", "I prefer minimal-friction approvals", "I dislike hidden background stalls". This should live in `arcwell-memory`, backed by `mem0-rs` and SQLite.

LLM Wiki is knowledge-shaped. It stores and searches source-backed corpora: 9000 pages on AI agents, 100 pages on developer relations, research briefs, source cards, contradiction notes, and imported documents. This should live in `arcwell-llm-wiki`, backed by its own SQLite corpus/index and Markdown wiki pages.
...
Recommendation: port this as the first large knowledge package, `arcwell-llm-wiki`, with `codex-memory-mcp` as the local MCP server inside the app package, not as a Codex fork.
...
- Preserve the boundary between personal memory and knowledge corpora. Built-in Codex Memories and `arcwell-memory` can remain enabled, but they are separate preference/identity layers, not a replacement for cited wiki knowledge.
...
- [nvk/llm-wiki](https://github.com/nvk/llm-wiki) explicitly describes a portable arcwell wiki methodology with Codex plugin support and commands such as `/wiki:ingest`.

### 3. LLM Wiki Freshness / Librarian / Interestingness

Recommendation: make `arcwell-llm-wiki` a living knowledge system, not just an ingest/search tool. It should watch curated sources, detect meaningful changes cheaply, enrich wiki pages through a librarian loop, score "interestingness", and send concise digests through concrete channel packages such as `arcwell-telegram` and `arcwell-mail`.
...
- `email-newsletter`: inbound newsletters routed through `arcwell-mail` or Cloudflare Email Workers into the same candidate ledger.
...
- `Usefulness to us`: whether it changes `arcwell-*` design, what to borrow, what to avoid.
...
- Always deliver through concrete channel packages (`arcwell-telegram`, `arcwell-mail`), not a generic push abstraction.
...
- Source health, recent errors, suppressed candidates, librarian findings, and delivery receipts are visible through MCP and `arcwell-ops-ui`.

### 4. Personal Memory

Recommendation: ship personal memory as `arcwell-memory`, backed by `mem0-rs` and SQLite, separate from `arcwell-llm-wiki`. This should be mem0-shaped as a full lifecycle memory system, not just CRUD tools.
...
- `thread_summary_capture`: controller summaries can be offered to memory as candidate personal preferences or recurring task facts, while wiki/source material is routed to `arcwell-llm-wiki`.
...
- Optional delivery through a concrete channel package such as `arcwell-telegram`.
...
Recommendation: implement Telegram as a channel bridge backed by `arcwell-controller`, not as an MCP-only integration and not as a one-workspace bot.
...
Recommendation: build `arcwell-channel-kit` as a shared contract, then keep each platform in its own concrete package. Do not create a generic `arcwell-push`.
...
- `arcwell-telegram`: first implementation because it is already a top-priority channel.
- `arcwell-discord`: future package, should reuse channel kit but own Discord-specific auth, embeds, threads, guild/channel policy, and rate limits.
- `arcwell-slack`: future package, should reuse channel kit but own Slack app auth, channels, threads, Block Kit, app mentions, and workspace policy.
- `arcwell-mail`: if email is used as a channel, it should reuse channel kit but own MIME, threading, reply quoting, loop prevention, and sender verification.
...
Recommendation: ship this as `arcwell-deep-research`, primarily a skill package plus optional custom agents, using the host agent's native web search as the default research substrate. Do not build a new research daemon first.
...
- Optional MCP tools/resources from `arcwell-llm-wiki`: `wiki_search`, `wiki_read`, `wiki_ingest`, `wiki_job_status`, and `wiki://page/{id}`.
- Optional `arcwell-search-kit` adapters for Brave Search and Perplexity Search/Sonar when extra recall, an independent index, cross-agent parity, or a second opinion is useful.
...
- Keep the `arcwell-deep-research` package as the first proof point before building any standalone workflow engine.
...
- Create the first `arcwell-*` skeleton with `worker/`, `local/`, `mcp/`, `hosts/codex/`, `hosts/claude/`, `docs/`, and tests.
- Decide whether to start as a monorepo (`arcwell-apps`) or separate repos. My default: monorepo until the shared envelope/drain contract stabilizes, then split only if community adoption wants independent release cadence.
...
- Run a Codex app-server/SDK spike before committing to `arcwell-controller`: prove start thread, stream events, list/read status, resume/follow-up, and failure handling from the intended host adapter. If Rust-native access is unstable or undocumented, define a tiny JS/Node shim behind a stable local HTTP/MCP contract.
- Define the minimum happy path for open-source adopters: one-command local install of `arcwell-profile` + `arcwell-memory` + `arcwell-backup` + `arcwell-ops-ui` with no Cloudflare requirement.
...
- Package `arcwell-profile` as an inspectable profile/preference store for communication register, output preferences, recurring constraints, taste/style rules, sizing defaults, approval/friction preferences, and decision criteria.
- Package `arcwell-conversation-import` with dry-run, redaction, candidates, review/apply/reject, import ledger, and export/delete flows for Claude/Codex history.
- Package `arcwell-quality-kit` as shared skills/contracts for root-cause debugging, reproduce-then-fix, tests, citations, visual inspection, blocked-state notification, and uncertainty surfacing.
- Package `arcwell-backup`, `arcwell-secrets`, and `arcwell-cost` early enough that personal data, credentials, and always-on model spend are controlled before real use.
...
- Package `arcwell-memory` around `mem0-rs` with SQLite as the default store.
...
- Add `arcwell-ops-ui` MVP that mirrors the MCP-backed ops state for human visual inspection and browser-use workflows.
...
- Build `arcwell-controller` local service with project registry, thread registry, active-run monitor, summary index, and natural-language resolver.
...
- Build `arcwell-channel-kit` schemas and test fixtures for inbound envelopes, outbound intents, formatting, injection fencing, and delivery receipts.
...
- Import `/Users/chabotc/Desktop/STYLE.md` as the canonical style profile for `arcwell-content`.
- Link `arcwell-content` to `arcwell-profile` so writing voice, register, and output preferences are editable profile artifacts rather than repeated prompt text.
...
- Copy `/Users/chabotc/Projects/garderobe` into the monorepo as `arcwell-garderobe`, preserving the Cloudflare Worker + D1 + OAuth/DCR remote MCP shape.
...
- Wire `arcwell-garderobe` to `arcwell-profile`, `arcwell-decision-support`, and `arcwell-routine-context` so outfit advice can use taste rules, fit/sizing history, weather, calendar/formality, comfort constraints, rotation, and prior "what worked" decisions.
...
- Add `arcwell-browser-workflows` recipes with separate browser profile guidance, action ledgers, screenshots, and verification.
- Add `arcwell-media` ledger for images, slides, video scripts, generated assets, exports, and delivery receipts.
...
- Credential lifecycle needs an owner: token location, scopes, refresh, expiry, rotation, revocation, and failed-grant alerts should be visible through `arcwell-secrets`, `doctor`, and `ops_health()`.
...
- Personal memory and LLM Wiki are separate systems: `arcwell-memory`/`mem0-rs` for compact personal facts and preferences; `arcwell-llm-wiki` for source-backed knowledge corpora. Memory Wiki pages should be Markdown files on disk and browsed/edited with Obsidian. Operational state is MCP-first, with `arcwell-ops-ui` as a standalone local web UI for human visual inspection of ingestion, status, history, logs, health, recent errors, and config.
- Add `arcwell-profile` as the explicit personal operating manual and `arcwell-conversation-import` as the safe bridge from Claude/Codex history into reviewed profile, memory, wiki, and decision artifacts. Do not turn conversation exports into hidden prompt sludge.
- Add `arcwell-backup`, `arcwell-secrets`, and `arcwell-cost` as first-wave cross-cutting services, not later polish. The system is not trustworthy without backup/restore, credential health, and spend ceilings.
- Add `arcwell-decision-support`, `arcwell-routine-context`, and `arcwell-quality-kit` to cover the recurring personal-use shape from the Claude export: constrained recommendations, daily context, and verified outputs.
- Define a clear policy for Codex built-in Memories when `arcwell-memory` is active: either disable built-in Memories for overlapping scopes or keep them in a documented non-overlapping role, so two memory systems do not conflict silently.
- The broader product target is Codex as the daily work shell, with portable services around it for coding context, wiki/research, content/style/publishing, Google Workspace, social/news/GitHub/paper monitoring, Telegram/email reachability, media/decks/video, and personal-domain MCP apps such as `arcwell-garderobe`.
- `/Users/chabotc/Desktop/STYLE.md` should become the canonical `arcwell-content` style profile. `/Users/chabotc/Projects/garderobe` should be copied into the monorepo as `arcwell-garderobe`, preserving its Cloudflare Worker + D1 + OAuth/DCR MCP design.
...
- Open-source `arcwell-*` packages provide skills, MCP servers, local drains, and optional Cloudflare Workers.
...
The first concrete milestone should be `arcwell-profile` + `arcwell-conversation-import` + `arcwell-memory`, because it proves the personal operating layer with inspectable preferences, safe history migration, `mem0-rs`, SQLite, MCP, and review/delete flows. The first large knowledge milestone should be `arcwell-llm-wiki`, because it unlocks cited corpus search for regular Codex and gives X/Telegram/mail/deep-research somewhere durable to write. The next milestone should be `arcwell-deep-research`, because it proves the regular-Codex subagent path before any new workflow engine is built. After that, build `arcwell-garderobe` and `arcwell-telegram`/`arcwell-x` to prove both the personal-domain MCP path and the edge-inbox/local-drain model against real events.
