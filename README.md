...
Current scoring and backlog clustering are intentionally transparent and simple. `arcwell knowledge cluster-backlog` can split unclustered source cards into deterministic entity/theme clusters, and `arcwell knowledge schedule-backlog-clustering` can let the resident worker run that local clustering before the existing wiki/report/digest expansion path. The backlog clusterer now preserves a signal mix for each cluster: provider families, source types, presentation roles, GitHub repos, external domains, and primary-vs-reaction counts, so reports can explain connections such as a repo release, social announcement, and community discussion without dumping raw links. `/ops/ui` has authenticated local Knowledge Controls for scheduling backlog clustering, enqueueing one-shot backlog clustering, queueing due cluster expansions, queueing due source-linked investigation execution, scheduling/enqueueing review-only model-cluster and model-writer jobs, and scheduling/enqueueing review-only entity-resolution jobs. Completed source-adapter jobs now record whether fresh source-card output chained into a pending backlog clustering job when an active `knowledge_backlog` watch source exists, including policy-blocked or active-job-skipped outcomes. Completed backlog jobs can visibly enqueue cluster expansion, and completed expansion jobs can visibly enqueue source-linked investigation execution, so one local worker pass can process backlog -> expansion -> investigation execution for a single eligible cluster when policy and worker capacity allow. Copied-home production-corpus proofs over 500 real source cards produced 12 source-backed human-readable reports, require each report to include explicit next-investigation steps, create source-card-linked research workflows when a cluster is expanded, execute those linked investigation tasks into durable source-card-cited research artifacts, and prove the worker can discover/execute eligible investigation tasks after expansion when policy allows enqueue. Production-data semantic clustering, broad live fresh primary-source acquisition, model-backed synthesis, and wall-clock recurrence remain later proof gates.
