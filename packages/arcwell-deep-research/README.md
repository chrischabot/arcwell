...
- Provider fallback for convergence challenges can enqueue bounded research-scoped URL ingests for selected safe results. When those worker jobs complete, Arcwell validates same-run host-search context, writes a source-card-backed full-text run source, records conservative deterministic claims when the readable text is safe to extract, and backfills the host-search result with the source-card id. This is source-reading plumbing, not a claim that every provider result has been publication-grade extracted.
- Local CSV/TSV/XLSX extraction creates document/table/cell artifacts with byte hashes, extractor version, formula-injection protection, formula preservation as untrusted text, stable row/column anchors, hidden/very-hidden worksheet skip warnings, merged-cell metadata with lowered confidence, and normalized XLSX date-time metadata. PDF text extraction uses a bounded local `pdftotext` helper when available; PDF layout table candidates are emitted with explicit heuristic warnings and confidence scores. Wrapped-header, irregular-column, and footnoted numeric PDF table fixtures lower confidence and preserve footnote refs instead of looking like clean spreadsheet evidence. Malformed, scanned, encrypted, unsupported, or failed documents record blocked/warning states instead of evidence.
...
6. Ingest and link source cards with `source_card_add` plus `run_id` or `research_source_card_link`; when using convergence provider fallback, drain any queued research-scoped `ingest_url` jobs before treating selected provider URLs as read evidence.
...
- Broader difficult-document fixtures: encrypted/scanned PDFs, rotated PDF tables, external footnote-heavy statistical tables, and an external workbook corpus beyond the current local XLSX hidden/merged/date fixture and wrapped-header/footnote PDF fixture.
