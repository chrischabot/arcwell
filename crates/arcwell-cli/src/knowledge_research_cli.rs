...
pub(crate) fn knowledge(store: Store, args: KnowledgeCommand) -> Result<()> {
    match args.command {
        ...
        KnowledgeSubcommand::ScheduleWeeklyOverview {
            name,
            channel,
            recipient_ref,
            time_zone,
            ...
            metadata: serde_json::json!({
                "cadence": "weekly",
                "weekday": weekday.trim().to_ascii_lowercase(),
                "window_hours": 24 * 7,
                "max_reports": max_reports,
                "max_source_cards": max_source_cards,
                ...
                "editorial_intent": "Friday morning end-of-week overview of the big stories and how they developed over the week",
                "created_from": "arcwell-cli knowledge schedule-weekly-overview"
            }),
            status: Some(status),
        })?),
