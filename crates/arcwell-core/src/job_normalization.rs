...
pub(crate) fn job_json_looks_like_lever(value: &Value) -> bool {
...
pub(crate) fn job_lever_lists_text(posting: &Value) -> Option<String> {
...
pub(crate) fn job_json_string_at(value: &Value, path: &[&str]) -> Option<String> {
...
pub(crate) fn job_json_bool_at(value: &Value, path: &[&str]) -> Option<bool> {
    let mut current = value;
    for key in path {
        current = current.get(*key)?;
    }
...
pub(crate) fn job_json_string_list_at(value: &Value, path: &[&str]) -> Vec<String> {
    let mut current = value;
    for key in path {
...
pub(crate) fn job_json_readable_text(value: &Value) -> String {
    let mut parts = Vec::new();
...
pub(crate) fn job_join_unique_strings<I>(parts: I) -> Option<String>
where
...
pub(crate) fn job_join_unique_optional_strings<I>(parts: I) -> Option<String>
where
...
pub(crate) fn job_report_entry_score(entry: &JobShortlistEntry) -> f64 {
    entry
        .score
        .as_ref()
...
pub(crate) fn job_report_location_label(role: &JobRoleCard) -> Option<String> {
...
pub(crate) fn job_report_role_family_key(role: &JobRoleCard) -> String {
    format!(
        "{}::{}",
