...
pub const APP_NAME: &str = "agent-services";
...
impl AppPaths {
    pub fn new(home: impl Into<PathBuf>) -> Self {
        let home = home.into();
        Self {
            db: home.join("agent-services.sqlite3"),
    ...
    pub fn from_env_or_default() -> Result<Self> {
        if let Ok(home) = std::env::var("AGENT_SERVICES_HOME") {
            return Ok(Self::new(home));
        }

        let home = std::env::var("HOME").context("HOME is not set")?;
        Ok(Self::new(PathBuf::from(home).join(".agent-services")))
    }
...
impl Store {
    ...
    pub fn create_backup(&self) -> Result<PathBuf> {
        ...
        let db_dest = dest.join("agent-services.sqlite3");
    ...
    pub fn x_oauth_exchange_code(
        ...
        let endpoint = std::env::var("AGENT_SERVICES_X_API_BASE")
...
fn default_worker_id() -> String {
    format!("agent-worker-{}", std::process::id())
}
...
fn validate_fetch_url(raw: &str) -> Result<Url> {
    ...
    if url.scheme() != "https" {
        if is_loopback_host(&url)
            && std::env::var("AGENT_SERVICES_ALLOW_LOOPBACK_URL_INGEST").as_deref() == Ok("1")
        {
            return Ok(url);
        }
...
fn fetch_text(url: &str, bearer_token: Option<&str>) -> Result<String> {
    ...
    let mut request = client
        ...
        .header("user-agent", "agent-services/0.1");
...
fn validated_endpoint(configured: Option<&str>, default: &str) -> Result<Url> {
    ...
    if configured.is_some()
        ...
        && std::env::var("AGENT_SERVICES_ALLOW_CUSTOM_SEARCH_ENDPOINTS").as_deref() != Ok("1")
    {
        bail!(
            "custom non-loopback search endpoints are disabled; set AGENT_SERVICES_ALLOW_CUSTOM_SEARCH_ENDPOINTS=1 to allow"
        );
    }
    Ok(url)
}
...
mod tests {
    ...
    fn test_store(name: &str) -> Store {
        let root =
            std::env::temp_dir().join(format!("agent-services-test-{name}-{}", Uuid::new_v4()));
    ...
    fn wiki_jobs_record_file_ingest_and_expand() {
        ...
        fs::write(&source, "# Job Source\n\nA launch about agent ops.").unwrap();
        ...
        store
            .add_source_card(SourceCardInput {
                ...
                url: "https://example.com/agent-ops".to_string(),
                source_type: "blog".to_string(),
                provider: "test".to_string(),
    ...
    fn severe_worker_migrates_legacy_job_schema() {
        let root = std::env::temp_dir().join(format!(
            "agent-services-test-legacy-worker-{}",
            Uuid::new_v4()
        ));
        fs::create_dir_all(&root).unwrap();
        let db = root.join("agent-services.sqlite3");
