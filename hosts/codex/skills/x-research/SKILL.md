...
- Use `x_oauth_authorize_url`, `x_oauth_exchange_code`, and `x_oauth_refresh` when live API access needs setup. Use `x_oauth_probe` to prove the current credential reaches the provider endpoints needed for `users.read`, `bookmark.read`, `follows.read`, and `tweet.read`. Use `x_oauth_revoke` only for explicit credential cleanup or recovery, because provider-side revoke is destructive.
